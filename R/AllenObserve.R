...
#' @param chains a list of lists, each with two elements, each of which
#' is a vector of chains.
#' @param ... Named arguments to be passed on to \code{allen_plot()}.
#'
#' @return An object of class archaeophases_plot.
#'
#' @examples
#'
#' \dontrun{
...
#' chains <- list(list(be3.amber, be1.dghnt), list(be1.dghnt, be3.amber))
#'
...
#' # allen_observe(data = oxc, chains = chains)
#' }
#' 
#' @export

allen_observe <- function(data, chains,  ...) {
  ...
  if(is.element("archaeophases_plot", class(data)))
    ...
    result_df <- allen_relate_intervals(mcmc = data,
                                        chains = chains)

  result_plot <- allen_plot(result_df, ...)
