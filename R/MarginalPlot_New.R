...
MarginalPlot <- function(a_chain, level=0.95, title="Characteristics of a date", subtitle = NULL,
  ...
  # credible interval
  CR <- CredibleInterval(a_chain, level)
  CR <- c(CR,"y"=0,"yend"=0)
  # Mean 
  Mean = mean(a_chain)
  # new dataframe
  ...
  h <- h + ggplot2::geom_segment(data = dataCR, aes(x=dataCR[1,1], xend=dataCR[1,2], y = dataCR[1,3], yend = dataCR[1,4], colour = '#E7B8OO'), size=3.95, show.legend=F) 
  h <- h + ggplot2::geom_point(data = dataCR,aes(x=Mean, y = dataCR[1,3]), size = 2)
  h <- h + scale_color_manual(values =c('#56B4E9', '#FC4EO7'))
