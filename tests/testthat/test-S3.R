...
test_that("Method original_file identifies default BCal import file", {
    bcal_1 <- read_bcal("test-data/bcal-1.csv")
...
test_that("Method original_file identifies arbitrary BCal import file", {
    bcal_17 <- read_bcal("test-data/bcal-17.csv")
...
test_that("Plot works with marginal_plot", {
    ...
    mp2 <- plot(mp)
    expect_equal(sum(mp), sum(mp2))
    expect_true(original_file(mp2))
    expect_false(original_file(mp2, "foo.csv"))
})
